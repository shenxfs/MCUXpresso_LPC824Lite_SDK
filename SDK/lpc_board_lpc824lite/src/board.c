...
static void Board_Key_Init(void)
{
	...
	for (i = 0; i < BOARD_KEY_CNT; i++) {
		Chip_GPIO_SetPinDIR(LPC_GPIO_PORT, 0, cs_keyBits[i], 0);//Chip_GPIO_PinSetDIR(LPC_GPIO_PORT, 0, cs_keyBits[i], 0);
...
static void Board_LED_Init(void)
{
	...
	for (i = 0; i < BOARD_LED_CNT; i++) {
		Chip_GPIO_SetPinDIR(LPC_GPIO_PORT, 0, ledBits[i], 1);//Chip_GPIO_PinSetDIR(LPC_GPIO_PORT, 0, ledBits[i], 1);
		Chip_GPIO_SetPinState(LPC_GPIO_PORT, 0, ledBits[i], true);//Chip_GPIO_PinSetState(LPC_GPIO_PORT, 0, ledBits[i], true);
	}
}
...
void Board_LED_Set(uint8_t LEDNumber, bool On)
{
	if (LEDNumber < BOARD_LED_CNT) {
		Chip_GPIO_SetPinState(LPC_GPIO_PORT, 0, ledBits[LEDNumber], (bool) !On);//Chip_GPIO_PinSetState(LPC_GPIO_PORT, 0, ledBits[LEDNumber], (bool) !On);
	}
}
...
bool Board_LED_Test(uint8_t LEDNumber)
{
	...
	if (LEDNumber < BOARD_LED_CNT) {
		state = (bool) !Chip_GPIO_GetPinState(LPC_GPIO_PORT, 0, ledBits[LEDNumber]);//state = (bool) !Chip_GPIO_PinGetState(LPC_GPIO_PORT, 0, ledBits[LEDNumber]);
	}
...
{
	if (LEDNumber < BOARD_LED_CNT) {
		Chip_GPIO_SetPinToggle(LPC_GPIO_PORT, 0, ledBits[LEDNumber]);//Chip_GPIO_PinToggleState(LPC_GPIO_PORT, 0, ledBits[LEDNumber]);
	}
}
